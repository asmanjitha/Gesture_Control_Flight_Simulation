...
namespace CoachAiEngine.Activities {
    ...
    [AddComponentMenu("Coach-AI Engine/Activities/Human Body/Pose3DARController")]
    public class Pose3DARController : CoachActivityController {

#if UNITY_EDITOR
        public void OnValidate() {
            if (CoachActivityConfiguration != null && !(CoachActivityConfiguration is Pose3DARSpec)) {
                Debug.LogWarning($"Configuration must be of type {nameof(Pose3DARSpec)}");
        ...
        protected override void InitializeSubscriptions() {
            ...
            foreach (var subscription in EventSubscriptions) {
                ...
                if (subscriptionId < 0) {
                    Debug.LogError($"{nameof(Pose3DARController)}: Could not subscribe to {subscription.Event}");
                }
            }
            ...
            foreach (var subscription in FeedbackSubscriptions) {
                ...
                if (subscriptionId < 0) {
                    Debug.LogError($"{nameof(Pose3DARController)}: Could not subscribe to {subscription.Feedback}");
                }
            }
