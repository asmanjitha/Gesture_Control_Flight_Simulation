...
namespace CoachAiEngine.Activities {
    ...
    [AddComponentMenu("Coach-AI Engine/Activities/Ball Sports/MultiBallActivityController")]
    public class MultiBallActivityController : CoachActivityController {

#if UNITY_EDITOR
        public void OnValidate() {
            if (CoachActivityConfiguration != null && !(CoachActivityConfiguration is MultiBallActivitySpec)) {
                Debug.LogWarning($"Configuration must be of type {nameof(MultiBallActivitySpec)}");
        ...
        protected override void InitializeSubscriptions() {
            ...
            foreach (var subscription in EventSubscriptions) {
                ...
                if (subscriptionId < 0) {
                    Debug.LogError($"{nameof(MultiBallActivityController)}: Could not subscribe to {subscription.Event}");
                }
            }
            ...
            foreach (var subscription in FeedbackSubscriptions) {
                ...
                if (subscriptionId < 0) {
                    Debug.LogError($"{nameof(MultiBallActivityController)}: Could not subscribe to {subscription.Feedback}");
                }
            }
