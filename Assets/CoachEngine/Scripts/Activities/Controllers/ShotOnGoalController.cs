...
namespace CoachAiEngine.Activities {
    ...
    [AddComponentMenu("Coach-AI Engine/Activities/Ball Sports/ShotOnGoalController")]
    public class ShotOnGoalController : CoachActivityController {

#if UNITY_EDITOR
        public void OnValidate() {
            if (CoachActivityConfiguration != null && !(CoachActivityConfiguration is ShotOnGoalSpec)) {
                Debug.LogWarning($"Configuration must be of type {nameof(ShotOnGoalSpec)}");
        ...
        protected override void InitializeSubscriptions() {
            ...
            foreach (var subscription in EventSubscriptions) {
                ...
                if (subscriptionId < 0) {
                    Debug.LogError($"{nameof(ShotOnGoalController)}: Could not subscribe to {subscription.Event}");
                }
            }
            ...
            foreach (var subscription in FeedbackSubscriptions) {
                ...
                if (subscriptionId < 0) {
                    Debug.LogError($"{nameof(ShotOnGoalController)}: Could not subscribe to {subscription.Feedback}");
                }
            }
